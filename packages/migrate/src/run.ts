 ...
 *
 * Kept out of the bin entry so that importing it runs nothing: the entry
 * point has to run unconditionally (see cli.ts), and a module that does work
 * on import printed "no config found" into every test run that loaded it.
 */
export async function run(argv: readonly string[] = process.argv.slice(2)): Promise<number> {
  ...
  // Where the config lands decides how carried relative imports are spelled.
  const result = migrate(found.source, found.tool, found.path, { outFile: outPath })
