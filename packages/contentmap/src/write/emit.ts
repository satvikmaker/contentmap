...
export async function emit(
  ...
  // A counter as well as the pid: two writes racing for the same path shared a
  // temp filename, so one renamed it away and the other failed with ENOENT.
...
export async function emitCollection({
  ...
  const work = entries.map(entry => {
    ...
    // Belt and braces. The naming above is collision-free for every case seen
    // so far, but a residual one — a file literally named `a__b` beside a
    // directory `a/` holding `b` — would otherwise have two documents race for
